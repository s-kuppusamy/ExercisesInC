...
People who want to work with advanced data structures and threads often use it.
It is designed for people interested with working with GUIs. There are other libraries aside from GLib that can be used for non GUI applications. It might have been good to give it a name that is not confused with the GNU C Linux  library, but it is not a huge deal since the capitalization is different.

*  What are the most important pros and cons of using GLib, as opposed
to another library, or just using core C features?
It is useful because there are several features available with the package that offer more advanced and reliable control. However, there are several dependencies you need install first to be able to use it properly.

*  What do you have to do to compile and run a "Hello GLib" example?
You need to make sure all required dependencies are installed first. Then, you can run the three commands listed on the compile page of the first link. To compile and run an actual application, you need to tell the compiler where to find the GLib header files and libraries.
